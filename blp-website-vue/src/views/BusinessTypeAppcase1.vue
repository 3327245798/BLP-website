<template>
  <div class="total">
    ...
    <a   @click="showDraftChapter(record)" target="_parent">
      ...更多
       <a-icon type="down" />
    </a>
    </a-dropdown>
...
export default {
  data() {
    ...
    const draftChapterColumns=[
      ...
      {title: '分析剧本',dataIndex: 'serialization',key: 'serialization',scopedSlots: { customRender: 'dratfChapterSlot' },width:300},
      {title: '操作',dataIndex: '#',key: '#',scopedSlots: { customRender: 'action' },width:120},
      ...
      chapterService.getDraftChapterListByDraftId(
        ...
        },
        {
          onSuccess: (model, fModel, json) => {
            ...
            console.log(json);
            this.draftChapteData=json.data.rows;
            this.firstDraftChapter = json.data.rows[0];
            console.log(
              "根据剧本id找该剧本下的章节list,只取第一条数据以弹窗的形式进行展示"
            );
